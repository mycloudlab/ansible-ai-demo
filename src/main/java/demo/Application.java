...
public final class Application {
    ...
    static final class EmailValidationException extends RuntimeException {
        EmailValidationException() { super("Email address has an invalid format"); }
    }
    static int validate(String name,String email) {
        if(name==null||name.isBlank()||name.length()>100)return 400;
        validateEmail(email);
        return 201;
    }
    static void validateEmail(String email) {
        if(email==null||email.length()>160||!email.matches("[^@\\s]+@[^@\\s]+\\.[^@\\s]+"))
            throw new EmailValidationException();
    }
    static void dispatch(HttpExchange x)throws java.io.IOException {
        String id=UUID.randomUUID().toString();x.getResponseHeaders().set("X-Request-Id",id);
        try {handle(x,id);}
        catch(RuntimeException e){
            // HTTP boundary: business validation does not translate this exception to a client error.
            synchronized(System.err){System.err.println(java.time.Instant.now()+" event=UNHANDLED_EXCEPTION request_id="+id);e.printStackTrace(System.err);}
            reply(x,500,"Erro interno ao processar a solicitação","text/plain");
        }finally{x.close();}
    }
    ...
    static void handle(HttpExchange x,String id)throws java.io.IOException {
        try {
            ...
            if(path.equals("/")&&x.getRequestMethod().equals("GET")){reply(x,200,"""
                ...
                <form id="form" novalidate><label>Nome<input name="name" required maxlength="100"></label><label>E-mail<input name="email" type="email" required maxlength="160"></label><button>Cadastrar</button></form><pre id="result"></pre>
                <script>const preset=new URLSearchParams(location.search).get('email');if(preset!==null)document.querySelector('[name=email]').value=preset;document.querySelector('#form').onsubmit=async e=>{e.preventDefault();try{let r=await fetch('/people',{method:'POST',body:new URLSearchParams(new FormData(e.target))});document.querySelector('#result').textContent='HTTP '+r.status+'\\n'+await r.text()+'\\nID: '+r.headers.get('X-Request-Id')}catch(e){document.querySelector('#result').textContent='Serviço indisponível'}};</script></html>
                ""","text/html");return;}
            ...
            if(path.equals("/people")&&x.getRequestMethod().equals("POST")){
                ...
                var f=form(new String(bytes,StandardCharsets.UTF_8));int status=validate(f.get("name"),f.get("email"));
                if(status!=201){log("INPUT_REJECTED",id);reply(x,status,"Nome inválido","text/plain");return;}
    ...
    public static void main(String[] args)throws Exception {
        ...
        server.createContext("/",Application::dispatch);server.setExecutor(Executors.newFixedThreadPool(4));server.start();log("APP_STARTED","startup");
