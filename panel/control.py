...
def main(action):
    if action == 'status':
        print(json.dumps({'application': state('caixa-demo'), 'database': state('postgresql@16-main'), 'validation': (CONTROL/'validation-fault').exists(), 'oom_pending': (CONTROL/'oom-once').exists()})); return
    if action not in ('database', 'validation', 'oom', 'reset-validation'):
        raise ValueError('Unknown action')
    if action == 'reset-validation':
        (CONTROL/'validation-fault').unlink(missing_ok=True); return
    if state('caixa-demo') != 'active' or state('postgresql@16-main') != 'active' or (CONTROL/'validation-fault').exists() or (CONTROL/'oom-once').exists():
    ...
    if action == 'database':
        ...
        p = CONTROL/('oom-once' if action == 'oom' else 'validation-fault')
